    ...
    getInitData() {
        ...
        function setData(){
            if(goodsUnits && goodsSmuggles && goodsBrands && goodsClassData && activitiesType && goodsInfo && priceRules) {
                ...
                let bindName = '',
                    sameBrand = goodsInfo.basic_infos.brand_name;
                ...
                that.state.sameParam = {
                    goods_id: goodsInfo.basic_infos.goods_id,
                    type_id: goodsInfo.basic_infos.goods_type_id,
                    brand: sameBrand,
                    guige: goodsInfo.basic_attributes.specs,
                    xinghao: goodsInfo.basic_attributes.types
                };
                that.getGoodsSame();
                that.picSortBlur();
    ...
    */
    constraint(val, item, status, e) {
        ...
        if(status == -1) {  //型号;
            ...
            obj.format_type_id = val.format_type_id;
        }else if(status == -2) { //规格;
        ...
        let sameParam = this.state.sameParam;
        if(status == -1) {  //型号;
            data.basic_attributes.types = obj;
            sameParam.xinghao = obj;
            this.getGoodsSame();
        }else if(status == -2) {  //规格;
    ...
    //获取相似商品的接口;
    getGoodsSame() {
        let param = this.state.sameParam;
        if(!param.guige.values[0].value || !param.xinghao.values[0].value || !param.brand) return;
        H.server.goods_same(JSON.stringify(param), (res) => {
            if(res.code == 0) {
                this.setState({sameData: res.data});
            }else if(res.code == 10106) {
                H.overdue();
            }else {
                H.Modal(res.message);
    ...
    createSameXML() {
        let data = this.state.sameData;
        if(data.length <= 0) return null;
        return (
            <div className="same-wrap">
                <div className="same-hd">相似商品</div>
                <div className="same-bd">
                    {
                        data.map((val, index) => {
                            return (
                                <div key={index}>
                                    <span style={{marginRight: '10px'}}>{val.gname}</span>
                                    <span style={{marginRight: '10px'}}>{val.xinghao}</span>
                                    <span>{val.guigei}</span>
                                </div>
                            );
                        })
                    }
    ...
    render() {
        ...
        return (
            <div>
                <div className="goods-save-btn"><a className="btn btn-lg btn-default" data-method="save" onClick={this.saveGoodsInfo.bind(this)}>保存</a></div>
                            ...
                            <div className="col-xs-12">
                                <div className="base-info">
                                    <div className="clearfix">
                                        ...
                                        <div className="bd" style={{width: '165px'}}>

(function(){
    ...
    server.getShopGoodsType  = function(params, callback){
        return send('get', '/mock/getShopGoodsType.json', params , callback);
    } ;

    //2 更新我的商品的价格
    server.changePrice  = function(params, callback){
        return send('get', '/mock/changePrice.json', params , callback);
    } ;

    //3 获取商品规格及型号约束
    server.getBasicAttr  = function(params, callback){
        return send('get', '/mock/getBasicAttr.json', params , callback);
    } ;

    //4 获取商品详情
    server.getGoodsInfo  = function(params, callback){
        return send('get', '/mock/getGoodsInfo.json', params , callback);
    } ;

    //5 获取特殊属性列
    server.getSpecialAttrList  = function(params, callback){
        return send('get', '/mock/getSpecialAttrList.json', params , callback);
    } ;

    //6 商品计量单位获取
    server.getUnits  = function(params, callback){
        return send('get', '/mock/getUnits.json', params , callback);
    } ;

    //7 商品修改
    server.updateGoods = function(params, callback){
        return send('get', '/mock/updateGoods.json', params , callback);
    } ;

    //8 商品添加
    server.addGoods = function(params, callback){
        return send('get', '/mock/addGoods.json', params , callback);
    } ;

    //9 获取价格体系规则
    server.getPriceRules = function(params, callback){
        return send('get', '/mock/getPriceRules.json', params , callback);
    }
})();
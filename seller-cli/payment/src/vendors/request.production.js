(function(){
    ...
    server.getPayment = function (params, callback) {
        return send('get', '?m=seller&c=Withdraw&a=getIndexInfo', params, callback);
    };
    // 获取用户角色
    server.getUserRole = function (params, callback) {
        return send('get', '?m=Member&c=Shop&a=getInfo', params, callback);
    };
    // 获取提现记录
    server.getRecode = function (params, callback) {
        return send('get', '?m=seller&c=Withdraw&a=getWithdrawLogList', params, callback);
    };
    // 获取已提现订单
    server.getWithdraw = function (params, callback) {
        return send('get', '?m=seller&c=Withdraw&a=getWithdrawOrderList', params, callback);
    };
    // 获取未完成订单信息
    server.getIncomplete = function (params, callback) {
        return send('get', '?m=seller&c=Order&a=getUnfinishedOrder', params, callback);
    };
    // 获取可提现订单信息
    server.getCan = function (params, callback) {
        return send('get', '?m=seller&c=Order&a=getFinishedOrder', params, callback);
    };
    // 获取提现中订单信息
    server.getWithdrawing = function (params, callback) {
        return send('get', '?m=seller&c=Withdraw&a=beingProcess', params, callback);
    };
    // 获取对账单
    server.getStatement = function (params, callback) {
        return send('get', '?m=seller&c=Withdraw&a=checkSlip', params, callback);
    };

    // 添加或更新银行卡
    server.updateCard = function (params, callback) {
        return send('post', '?m=seller&c=Withdraw&a=bankCartEdit', params, callback);
    };
    // 查看银行卡信息
    server.showCard = function (params, callback) {
        return send('get', '?m=seller&c=Withdraw&a=getBankCartInfo', params, callback);
    };

    // 申请提现
    server.withdraw = function (params, callback) {
        return send('post', '?m=Home&c=MyOrderWithdraw&a=getWithdrawalsApply', params, callback);
    };
})();